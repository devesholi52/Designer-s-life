...
export default function MyOrdersDelivered({ navigation, route }) {
    ...
    const RejectOrderForDeal = async () => {
        const token = await AsyncStorage.getItem('token')
        let form = new FormData()
        form.append('order', dealDetail?.id)
        fetch(Appurl.ORDER_CANCEL, {
            ...
            .then(response => {
                if (response && response.data && response.data.order_rejected == true) { showToastMessage('Deal cancelled ') }
                navigation.navigate("MyOrderScreen")
            })
    }
    if (!dealDetail) {
        return (
            <ScrollView>
                ...
                {/* <View style={styles.imageText}>
                    <Text style={styles.TotalEarning}>Place Order </Text>
                ...
                <TouchableOpacity style={{ backgroundColor: '#b22222', paddingVertical: 13, marginHorizontal: 30, borderRadius: 8, marginBottom: 30, elevation: 3 }}
                    onPress={() => RejectOrder()}>
                    <Text style={{ alignSelf: 'center', fontFamily: Fonts.RobotoBold, color: 'white', fontSize: 18 }}>Cancel Order</Text>
                </TouchableOpacity>
        ...
        return (
            <ScrollView>
                <TouchableOpacity style={{ marginBottom: 30 }} >
                    <View style={{ flexDirection: 'row', marginLeft: 10, marginTop: 15 }}>
                        ...
                        <View style={{ flex: 3, marginTop: 5 }}>
                            ...
                            {/* <Text style={{ fontSize: 12, fontFamily: Fonts.RobotoBold, paddingTop: 5 }}>{dealDetail?.deal_description}</Text> */}
                            <View style={{ flexDirection: 'row', marginVertical: 10 }}>
                                <Text style={{ color: Colors.black, fontSize: 12, fontFamily: Fonts.RobotoMedium }}>{dealDetail?.deal_price}</Text>
                                <Text style={{ textDecorationLine: 'line-through', textDecorationStyle: 'solid', marginHorizontal: 10, fontSize: 12, color: Colors.app_grey, fontFamily: Fonts.RobotoMedium }}>
                                    {Math.trunc(dealDetail?.total_price)}
                                </Text>
                            ...
                            {/* <Text style={{ color: 'black', fontFamily: Fonts.RobotoRegular, marginBottom: 10 }}>Delivered by 28feb 2022</Text> */}
                            <Text style={{ color: Colors.black, fontSize: 12, fontFamily: Fonts.RobotoRegular }}>{date ? moment(date).format('MMM Do YYYY') : ''}</Text>
                ...
                <View style={styles.containerTotalamount}>
                    <Text style={{ fontFamily: Fonts.RobotoBold, color: 'black' }}>Total Amount</Text>
                    <Text style={{ fontFamily: Fonts.RobotoMedium, color: 'black' }}>{Math.trunc(dealDetail?.deal_price)}</Text>
                </View>

                <TouchableOpacity style={{ backgroundColor: '#b22222', paddingVertical: 13, marginHorizontal: 30, borderRadius: 8, marginBottom: 30, elevation: 3 }}
                    onPress={() => RejectOrderForDeal()}>
                    <Text style={{ alignSelf: 'center', fontFamily: Fonts.RobotoBold, color: 'white', fontSize: 18 }}>Cancel Deal</Text>
                </TouchableOpacity>
                {/* </View> */}
                {/* <View style={styles.bottomtextcontainer}>
                            <Text style={{ paddingVertical: 5, fontFamily: Fonts.RobotoRegular }}>24 Sept,2021</Text>
