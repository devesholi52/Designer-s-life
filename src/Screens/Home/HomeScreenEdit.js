...
const HomeScreenEdit = ({ navigation, route, onSubmit = () => { } }) => {
        ...
        >
            ...
            <View style={styles.Container} >
                ...
                <FlatList
                    ...
                    horizontal={true}
                />
                <TouchableOpacity style={{ alignSelf: 'center', paddingHorizontal: 40, paddingVertical: 10, backgroundColor: selectedArr.length == "0" ? `#dcdcdc` : "black", borderRadius: 10, marginVertical: 15 }}
                    onPress={() => onSubmit(selectedArr)}
                    disabled={selectedArr && selectedArr.length == 0}>
                    <Text style={{ fontSize: 15, color: selectedArr.length == "0" ? Colors.dark_grey : "white", fontFamily: Fonts.RobotoMedium }}>Proceed</Text>
                </TouchableOpacity>
                {products && products?.length !== 0 && <FlatList
                    data={products}
                    renderItem={({ item, index }) => <HomeProductEdit
                        refreshPage={() => {
