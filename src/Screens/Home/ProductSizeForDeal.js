...
export default function ProductSizeForDeal({ navigation, itemData = null, submitSize = null, cloth = true, }) {
    const [selectedIndex, setSelectedIndex] = useState(null)
    const submit = (item, selectedIndexValue) => {
        setSelectedIndex(selectedIndexValue);
        submitSize(item)
    }
    // console.log('What is in Size', itemData);
    return (
                ...
                <FlatList
                    ...
                    renderItem={({ item, index }) => {
                        return (<TouchableOpacity style={[style.shodow, style.sizeBox, {
                            ...
                            <Text style={style.sizeText}>{item?.size ? item?.size : '3'}</Text>
                        </TouchableOpacity>)
                    }} />
            </View>
        </View>
