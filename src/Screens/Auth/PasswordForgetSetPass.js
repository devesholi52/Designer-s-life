...
import { Image, Text, TextInput, TouchableOpacity, View, StyleSheet, ImageBackground, ScrollView } from "react-native";
...
export default OTPScreen = ({ route }) => {
    ...
    const Submit = () => {
        ...
        fetch(Appurl.OTP_PASS_CHANGE, {
            ...
            .then(response => {
                if (response && response.message == "You have successfully verified your account.") {
                    showToastMessage("OTP verified.")
                    navigation.replace("ResetPassFromSignIn",
                        {
                            Email: userdata
                        }
                    )
                }
                if (response && response.message == "The OTP you entered is not valid please try again.") {
    ...
    return (
        ...
        <ScrollView
            ...
            <Image
                ...
                style={{ height: 100, width: 250, marginBottom: 20, alignSelf: 'center', marginTop: width / 4.5 }} />
            <View style={{ marginHorizontal: 5, marginVertical: 20, alignSelf: 'center' }}>
                ...
                <Text style={{ fontFamily: Fonts.RobotoBold, color: Colors.white, fontSize: 11, alignSelf: 'center', marginTop: 10 }}>An 4 digits passcode sent to </Text>
                <Text style={{ fontFamily: Fonts.RobotoBold, color: Colors.white, fontSize: 11, alignSelf: 'center', marginTop: 5 }}>{userdata}</Text>
            </View>
