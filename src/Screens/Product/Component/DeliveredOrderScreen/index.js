...
const DeliveredOrderScreen = ({ navigation }) => {
  ...
  const messageSeller = () => {
    ...
    //   return
    // }
    navigation.navigate('MessageOrderDetails', { userId: item?.created_by })
  }
              ...
              <View style={{ flexDirection: 'row', paddingVertical: 25 }}>
                <View style={{ flex: 4, justifyContent: 'center', paddingHorizontal: 5 }}>
                  <Pressable style={Styles.MainContainer}
                    onPress={() => navigation.navigate('DeliveredOrders', { item: item, date: item?.created_on, })}>
                    ...
                    {(item?.cart_product?.[0]?.product && item?.cart_product?.[0]?.product !== 0) ?
                      <View style={Styles.innerBox}>
                        ...
                        <Text style={Styles.ProductDescText} ellipsizeMode={"tail"} numberOfLines={1}>{item?.cart_product?.[0]?.product?.description}</Text>
          ...
          {(item?.deal_product && item?.deal_product !== 0) ?
            <View style={Styles.container}>
              <View style={{ flexDirection: 'row', paddingVertical: 25 }}>
                <View style={{ flex: 4, justifyContent: 'center', paddingHorizontal: 5 }}>
                  <Pressable style={Styles.MainContainer}
                    onPress={() => navigation.navigate('DeliveredOrders', { item: item })}>
                    {(item?.deal_product && item?.deal_product !== 0) ?
                      <Image style={Styles.img} source={{
                    ...
                    {(item?.deal_product && item?.deal_product !== 0) ?
                      <View style={Styles.innerBox}>
                        ...
                        <Text style={Styles.ProductCreatedText}>{item?.updated_on ? moment(item?.updated_on).format('DD/MM/YYYY') : ''}</Text>
                        {item?.deal_product && item?.deal_product?.length !== 0 &&
                          <Text style={Styles.ProductCreatedText}>{item?.deal_product?.[0]?.length} more</Text>}
                      </View>
                      : <Text>No Data available</Text>}
                  </Pressable>
                </View>
