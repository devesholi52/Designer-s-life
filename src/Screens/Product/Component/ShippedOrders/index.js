...
const ShippedOrders = ({ navigation }) => {
  ...
  const ConfirmOrder = async (id) => {
    ...
    let form = new FormData()
    form.append('order', id)
    form.append('order_status', "delivered")
    fetch(Appurl.CONFIRM_ORDER, {
      ...
      .then(response => {
        if (response && response?.status == 'True') {
          showToastMessage('Order Delivered ')
          getOrder()
        }
      })
  }
              ...
              <View style={{ flexDirection: 'row', paddingVertical: 25 }}>
                <View style={{ flex: 4, justifyContent: 'center', paddingHorizontal: 5 }}>
                  <Pressable style={Styles.MainContainer}
                    onPress={() => navigation.navigate('ShowShippedordersProduct', { item: item, date: item?.created_on, })}>
                        ...
                        <MenuOption>
                          <View >
                            ...
                            <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', marginVertical: 5 }}
                              ...
                              <Text style={Styles.ProductConfirmText}>Delivered</Text>
                            </TouchableOpacity>
                            {/* <TouchableOpacity
          ...
          {(item?.deal_product && item?.deal_product !== 0) ?
            <View style={Styles.container}>
              <View style={{ flexDirection: 'row', paddingVertical: 25 }}>
                <View style={{ flex: 4, justifyContent: 'center', paddingHorizontal: 5 }}>
                  <Pressable style={Styles.MainContainer}
                    onPress={() => navigation.navigate('ShowShippedordersProduct', { item: item })}>
                    {(item?.deal_product && item?.deal_product !== 0) ?
                      <Image style={Styles.img} source={{
