...
const ProductCardDeal = ({ item, navigation,
    index,
    onPressLike,
    myCustomShare,
    removeProduct,
    add_To_Favourite_Deal
}) => {
    const onShare = async () => {
        let id = item?.id
        ...
        let imageUrl = item?.deal_pictures[0]?.product_images ? `${Appurl.ROOT + item?.deal_pictures[0]?.product_images}` : staticImage
    ...
    return (
        <View style={style.box}>
            ...
            >
                {item?.deal_pictures.length === 1 ?
                    <Image source={{ uri: item?.deal_pictures[0]?.product_images ? Appurl.ROOT + item?.deal_pictures[0]?.product_images : staticImage }} style={style.singleImage} />
                    : <Image source={{ uri: item?.deal_pictures[0]?.product_images ? Appurl.ROOT + item?.deal_pictures[0]?.product_images : staticImage }} style={style.boxMainImage} />}

                {item?.deal_pictures?.length === 2 ?
                    <Image source={{ uri: item?.deal_pictures[1]?.product_images ? Appurl.ROOT + item?.deal_pictures[1]?.product_images : staticImage }} style={style.boxSecondImage} />
                    : <>
                        {item?.deal_pictures?.length > 2 && <View style={style.rightImgBox}>
                            <Image source={{ uri: item?.deal_pictures[1]?.product_images ? Appurl.ROOT + item?.deal_pictures[1]?.product_images : staticImage }} style={style.boxSecondHalfImage} />
                            {<View>
                                {item?.deal_pictures?.length > 3 && <Text style={style.imagesCount}>+{item?.deal_pictures?.length - 3}</Text>}
                                <Image source={{ uri: item?.deal_pictures[2]?.product_images ? Appurl.ROOT + item?.deal_pictures[2]?.product_images : staticImage }} style={[style.boxThirdImage, {
                                    opacity: item?.deal_pictures?.length > 3 ? 0.7 : 1.0
                                }]} />
                            </View>}
                        </View>}
                    </>}
            ...
            <View style={style.boxBottom}>
                <View style={{ paddingHorizontal: 5, }}>
                    <TouchableOpacity onPress={() => navigation.navigate("ProductDetailsDeal", {
                        ...
                        <Text
                            style={style.TittleText}>{item?.title}</Text>
                    </TouchableOpacity>
                    <Text
                        style={style.DescriptionText}>{item?.description}</Text>
                    ...
                    <View style={{ flexDirection: 'row', marginVertical: 5 }}>
                        ...
                        <Text
                            style={style.DiscountText}>{Math.trunc(100 - item?.price * 100 / item?.total_price)}%</Text>
                    </View>
                </View>
